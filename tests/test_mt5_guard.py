...
def test_bridge_headers_include_ngrok_skip():
    # Always present so ngrok's free-tier interstitial is bypassed.
    assert _bridge_headers().get("ngrok-skip-browser-warning") == "true"


def test_bridge_headers_no_key_when_unconfigured(monkeypatch):
    monkeypatch.setattr(settings, "MT5_BRIDGE_API_KEY", "")
    assert "X-Bridge-Key" not in _bridge_headers()
